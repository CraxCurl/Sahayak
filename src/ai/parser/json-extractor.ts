import { PageAdaptationManifestSchema, PageAdaptationManifest } from '../schemas/page-adaptation.schema';
 ...
 * Satisfies Phase 2 (Restructured JSON Extractor).
 */
export function extractAndRepairJson(rawOutput: string): string {
 ...
 */
export function parseAndValidateGemmaOutput(rawOutput: string): PageAdaptationManifest {
  const cleaned = extractAndRepairJson(rawOutput);

  try {
    const rawJson = JSON.parse(cleaned);
    const parsed = PageAdaptationManifestSchema.parse(rawJson);
    return parsed;
  } catch (error) {
    console.error(
      '[Sahayak JSON Extractor] Schema validation / JSON parse error:',
      error,
      '\nCleaned string:',
      cleaned,
      '\nRaw input:',
    ...
    throw new Error(`JSON Extract/Schema Validation Failed: ${String(error)}`);
  }
}
